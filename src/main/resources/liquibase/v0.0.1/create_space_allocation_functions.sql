/
CREATE FUNCTION `LS_ALLOC_SPACE`(`REQ_MEDIA_TYPE` VARCHAR(128), `REQ_STORAGE_FILE_NAME` VARCHAR(64), `REQ_ALLOC_SIZE` BIGINT(20)) RETURNS VARCHAR(16)
BEGIN
	DECLARE `HISTORY_MARKER` VARCHAR(36) DEFAULT UUID();
	DECLARE `RS_STORAGE_NAME` VARCHAR(16) DEFAULT NULL;
	INSERT INTO `SPACE_ALLOCATIONS_HISTORY`(`GUID`, `STORAGE_FILE_NAME`, `STORAGE_NAME`, `OPERATION_TYPE`, `SIZE`)
	WITH
		`V_STORAGE` AS (
			SELECT
				`S`.`STORAGE_NAME`,
...
END
/
CREATE FUNCTION `LS_RELEASE_SPACE`(`REQ_STORAGE_NAME` VARCHAR(16), `REQ_STORAGE_FILE_NAME` VARCHAR(64)) RETURNS TINYINT(1)
BEGIN
	DECLARE `HISTORY_MARKER` VARCHAR(36) DEFAULT UUID();
	DECLARE `RS_SUCCEED` TINYINT(1) DEFAULT 0;
	INSERT INTO `SPACE_ALLOCATIONS_HISTORY`(`GUID`, `STORAGE_FILE_NAME`, `STORAGE_NAME`, `OPERATION_TYPE`, `ALLOCATION_REF_ID`)
	SELECT 
		`HISTORY_MARKER` AS `GUID`,
		`H`.`STORAGE_FILE_NAME`,
		`H`.`STORAGE_NAME`,
