package io.bincloud.common.domain.model.web;
...
public class KeyValueParameters {
	...
	public Collection<String> getValues(String headerName) {
		return Collections.unmodifiableList(Optional.ofNullable(httpHeaders.get(headerName)).orElse(Collections.emptyList()).stream()
				.filter(value -> value != null).map(String::trim).filter(value -> !value.isEmpty()).collect(Collectors.toList()));
	}
		...
		return Collections.unmodifiableList(getValues(headerName).stream().<Optional<T>>map(value -> {
			try {
				...
				logger.log(new LogRecord(Level.WARN,
						new TextMessageTemplate(HEADER_VALUE_PARSING_ERROR_LOG_MESSAGE_TEMPLATE)
