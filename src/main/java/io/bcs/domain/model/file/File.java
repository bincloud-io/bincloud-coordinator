...
public class File {
  ...
  private static FileStateCreator STATE_CREATOR =
      new FileStateCreator().registerStateFactory(FileStatus.DRAFT, FileDraftState::new)
  ...
  private static class FileStateCreator {
    ...
    public FileState createStateFor(FileStatus status, FileEntityAccessor accessor) {
      log.log(new LogRecord(Level.DEBUG,
          TextTemplates.createBy("Create ").withParameter("status", status)));
