...
public class Polling {
  ...
  public final <
      D> Stream<PolledElement<D>> sequentialNumeratedPolling(BatchPoller<D> pollingFunction) {
    return polling(pollingFunction, false);
  }

  public final <
      D> Stream<PolledElement<D>> parallelNumeratedPolling(BatchPoller<D> pollingFunction) {
    return polling(pollingFunction, true);
  }

  private final <D> Stream<PolledElement<D>> polling(BatchPoller<D> pollingFunction,
      boolean parallel) {
