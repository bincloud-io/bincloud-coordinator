...
public class TextTransformers {
	/**
	 * Create the transformer trimming the {@link TextTemplate#getTemplateText()}
	 * value
	 * 
	...
	/**
	 * Create the "deep dive" transformer, which applies the specified
	 * transformation to each parameter, implementing the {@link TextTemplate}
	 * interface and create derived text template with transformed parameters. Then
	 * it applies the transformation to the derived text template. It is important
	 * that the transformer traverses all parameters recursively (parameters of
	 * root, parameters of the parameters of root...).
	 * 
	 * @param transformer The non-recursive transformer applying to the message
	 *                    template
	 * @return The recursive deep-dive transformer applying the non-recursive
	 *         transformer to the parameters recursively
	 */
	...
	private static class TrimmingTransformer implements Transformer {
		...
		public TextTemplate transform(TextTemplate sourceTemplate) {
			return TextTemplates.createBy(sourceTemplate.getTemplateText().trim(), sourceTemplate.getParameters());
		}
	}
	...
	private static class ChainTransformer implements Transformer {
		...
		public TextTemplate transform(TextTemplate sourceTemplate) {
			return sourceTemplate
				.transformBy(originalTransformer)
				.transformBy(nextTransformer);
		}
	}
	...
	private static class DeepDiveTransformer implements Transformer {
		...
		private Map<String, Object> transformParametersRecursively(TextTemplate template) {
			return template.getParameters().entrySet().stream().collect(Collectors.toMap(Entry::getKey, entry -> {
				Object value = entry.getValue();
				if (value instanceof TextTemplate) {
					return ((TextTemplate) value).transformBy(new DeepDiveTransformer(transformer)).toString();
				}
				return value;
			}));
		}
